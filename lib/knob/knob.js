function setupKnob(knob) {
  ...
  class WobbleMode extends ElementUIMode {
    ...
    activate(e) {
      ...
      console.log(`Start wobbling:`, this.initialState, this.currentState);
    ...
    processWobbleChangeEvent(e) {
      if (e.detail.element == this.element) {
        ...
        this.calculateWobble();
        console.log("WobbleChange", this.currentState)
      }
    }
  ...
  class WobbleConfigMode extends ElementUIMode {
    ...
    activate(e) {
      ...
      console.log(`Start wobble config:`, this.initialState, this.currentState);
    }

    pointerMove(e) {
      //console.log("PointerMoved", e)
      if (this.isActive) {
        ...
        //this.noMovementTimer = setTimeout(() => this.noMovementDetected(), this.noMovementDuration);
        console.log("Move", this.initialState, this.currentState)
        //console.log(`X=${position.x}, Y=${position.y}, DX=${_wobbleConfigDeltaX}, DY=${_wobbleConfigDeltaY}, XR=${deltaXToWidthRatio}, YR=${deltaYToHeightRatio}, WCD=${_wobbleConfigNewDepth}, WCR=${_wobbleConfigNewRate}`);
      }
    }

    noMovementDetected() {
      console.log(`No movement detected for ${this.name}`);
      super.deactivate();
      console.log(`Called super.deactivate for ${this.name}`);
    }
  ...
  if (!knobInput) {
    console.error("Knob input element not found", knob);
    return;
  }
  ...
  if (isNaN(knobInput.min)) {
    console.error("Knob value is not a number", knobInput);
    return;
  }

  //console.log("Setup knob " + knob.name, knob);
  ...
  if (knobMin < 0) {
    //console.log("min knob", knob)
  }
  if (stepSize == 1) {
    //    console.log("Knob " + knob.getAttribute("name"), knobDefinition)
  }
  ...
  function startLongTouchTimer(e) {
    ...
    console.log("Start long touch timer");
  }
  ...
  function longTouch() {
    ...
    console.log("Long touch detected", _longTouchEvent);
    stopLongTouchTimer();
  ...
  function stopLongTouchTimer() {
    if (_longTouchTimer) {
      ...
      console.log("Cleared long touch timer");
    }
  }

  function startDoubleTapTimer() {
    _doubleTapTimer = setTimeout(doubleTapTimeElapsed, _doubleTapDuration);
    //console.log("Start double tap detection");
  }

  function stopDoubleTapTimer() {
    if (!_doubleTapTimer)
      return;
    //console.log(`Stop double tap timer for ${knobInput.name}`);
    clearTimeout(_doubleTapTimer);
    _doubleTapTimer = null;
  }

  function doubleTapTimeElapsed() {
    //console.log(`No double tap detected for ${knobInput.name} in ${_doubleTapDuration}`);
    stopDoubleTapTimer();
  }

  function doubleTap() {
   // console.log("Double tap detected for " + knobInput.name);
    clearTimeout(_doubleTapTimer);
    _doubleTapTimer = null;
  ...
  function decreaseValue(valueToDecrease) {
    ...
    //console.log(`decreaseValue with ${valueToDecrease}`);
  }
  ...
  function setKnobValue(value, angle) {
    ...
    if (knobValueLabel) {
      ...
      if (listName) {
        ...
        if (!list) {
          console.error("List " + listName + " not found for knob input", knobInput);
