function applyTemplates() {
  ...
  while (templateItems.length > 0) {
    //console.log(`Applying ${templateItems.length} template items`);
    ...
    //   console.log("Remaining template items " + templateItems.length, templateItems);
    // }
  }
}
...
function applyTemplate(element) {
  ...
  if (!itemTemplate) {
    console.error("Template " + element.dataset.template + " not found", element);
  ...
  //console.log("Applied template " + itemTemplate.id);//, element, templatedElement.outerHTML)
...
function applyControlTemplate(element, itemTemplate, templateContainer) {
  ...
  if (!templateContainer.firstChild) {
    console.error("Template " + element.dataset.template + " is empty", itemTemplate);
    return;
  }
  ...
  if (contentElement) {
    ...
    for (var i = 0; i < element.attributes.length; i++) {
      ...
      if (!templatedElement.getAttribute(name)) {
        //console.log("copying att " + name, value);
        templatedElement.setAttribute(name, value);
      }
      // else {
      //   console.log("skipping att " + name, value);
      // }
...
function formatHumanText(text) {
  ...
  //   //console.log(`Old text=${text}, new=${newText}`);
  // }
  return newText;
}
