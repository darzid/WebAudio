class Sampler {
  ...
  async requestAccess() {
    ...
    console.log("Microphone acccess aqcuired");
  }

  async startRendering(sourceNode) {
    console.log("startRendering[1] started.");
    this.isRecording = true;
    let blob = null;
    try {
      
     // this._renderedBuffer = this.context.createBuffer(2, this.context.sampleRate * 40, this.context.sampleRate);
    //  console.log("startRendering[2] renderedBuffer created");

      console.log("startRendering[2] starting offline context start rendering");
      let buffer = await this.context.startRendering();
      console.log("startRendering[3] completed successfully. result=", buffer);
    //  .then(audioBuffer => console.log("Rendering completed successfully. result=", audioBuffer));
      const wav = this._audioBufferToWav(buffer);
      blob = new Blob([wav], { type: 'audio/wav' });
      

    //  console.log("startRendering[4] Rendered");
     /* console.log("startRendering[5] created render buffer");
      sourceNode.disconnect();
      sourceNode.connect(offlineContext);
      sourceNode.start();*/
      //_console.log("startRendering[5] started sourceNode");
    }
    catch (error) {
      console.error("startRendering failed.", error);
      throw error;
    }
    ...
    console.log("Imported recording " + this.currentRecordingId);
  ...
  stopRendering() {
   // console.log("Trying to stop rendering", this._renderedBuffer);
    this.isRecording = false;
    //this._offlineContext.suspend(0);
    console.log("Rendering stopped", this._renderedBuffer);
   ...
   /* this._song = new AudioBufferSourceNode(this.context, { buffer: this._renderedBuffer });
    ...
    this._song.start();
    console.log("Playing rendered song.");*/
  }
  ...
  stopPlayback(sampleId) {
    ...
    try {
      ...
      console.error(`Error stopping playback source "${playingSample.playbackSourceId}":`, err);
    }
  }
  ...
  importSample(sampleId, blob) {
    ...
    console.log(`Imported sample: ${newSample.id}-${newSample.name} (${(newSample.blob.size / 1024).toFixed(2)} KB)`);
    return newSample;
  }
  ...
  loadSamplesFromState() {
    ...
    console.log(`Loaded ${this.samples.length} samples from state.`);
  }
  ...
  async saveSamplesToState() {
    ...
    console.log(`Saved ${samplesToSave.length} samples to state.`);
  }
