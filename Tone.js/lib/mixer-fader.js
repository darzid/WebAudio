customElements.define("mixer-fader", class MixerFader extends HTMLElement {
  constructor() {
    super();
  }
  ...
  connectedCallback() {
    ...
    this.module = {
      ...
      properties: {
        ...
        'class': { type: String, value: "mixer-fader" }
      },
    };
...
<input ${idAttribute} ${nameAttribute} type="number" class="${this.class}" min="${this.min}" max="${this.max}" step="${this.step}" value="${this.value}">`;
    ...
    this.drawFill = function() {
      ...
      let backgroundImage =
        `linear-gradient(to right, ${this.fill} 0%, ${this.fill} ${progress}%, ${this.backgroundColor} ${progress}%, ${this.backgroundColor} 100%)`;
